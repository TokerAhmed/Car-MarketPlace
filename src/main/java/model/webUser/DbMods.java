...
public class DbMods {
    ...
    private static StringData validate(StringData inputData) {
        ...
        errorMsgs.userImage = Validate.stringMsg(inputData.userImage, 600, false);
    ...
    public static StringData getByEmailPassword(DbConn dbc, String email, String password) {
        StringData sd = new StringData();
        
        if (email == null || password == null) {
            sd.errorMsg = "Cannot getByEmailPassword: email or password is null";
            return sd;
        }
    
        sd.errorMsg = dbc.getErr();
        if (sd.errorMsg.length() > 0) { 
            return sd; // return any database connection error
        }
    
        try {
            String sql = "SELECT web_user_id, user_email, user_password, membership_fee, birthday, "
                       + "user_image, web_user.user_role_id, user_role_type "
                       + "FROM web_user INNER JOIN user_role ON web_user.user_role_id = user_role.user_role_id "
            ...
            if (results.next()) {
                ...
                sd.errorMsg = "Web User Not Found or Password incorrect.";
            }
            results.close();
            stmt.close();
        } catch (Exception e) {
            sd.errorMsg = "Exception in getByEmailPassword(): " + e.getMessage();
        }
        return sd;
    }//getByEmailPassword
